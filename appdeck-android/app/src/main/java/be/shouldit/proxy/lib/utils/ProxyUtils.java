...
//            APL.getEventsReporter().sendEvent(e);
    ...
    public static String getSecurityString(SecurityType security, PskType pskType, Context context, boolean concise)
    {
        switch (security)
        {
            ...
            case SECURITY_NONE:
            default:
                return concise ? "wifi_security_none" : "wifi_security_none";
        }
    }
...
//        LogWrapper.d(TAG, "Sending broadcast intent: " + APLConstants.APL_UPDATED_PROXY_STATUS_CHECK);
    ...
    protected static ProxyStatusItem isWifiEnabled(WiFiApConfig conf)
    {
        ...
        if (APL.getWifiManager().isWifiEnabled())
        {
            ...
            {
                String status = "status_wifi_enabled";
                result = new ProxyStatusItem(ProxyStatusProperties.WIFI_ENABLED, CheckStatusValues.CHECKED, true, true, status);
            }
            else
            {
                result = new ProxyStatusItem(ProxyStatusProperties.WIFI_ENABLED, CheckStatusValues.CHECKED, false, true, "status_wifi_enabled_disconnected");
            }
        }
        else
        {
            result = new ProxyStatusItem(ProxyStatusProperties.WIFI_ENABLED, CheckStatusValues.CHECKED, false, true, "status_wifi_not_enabled");
        }
    ...
    protected static ProxyStatusItem isWifiSelected(WiFiApConfig conf)
    {
        ...
        if (conf.isActive())
        {
            result = new ProxyStatusItem(ProxyStatusProperties.WIFI_SELECTED, CheckStatusValues.CHECKED, true, true, "status_wifi_selected"/*, conf.getSSID()*/ );
        }
        else
        {
            result = new ProxyStatusItem(ProxyStatusProperties.WIFI_SELECTED, CheckStatusValues.CHECKED, false, true, "status_wifi_not_selected");
        }
    ...
    protected static ProxyStatusItem isProxyEnabled(WiFiApConfig conf)
    {
        ...
        if (Build.VERSION.SDK_INT >= 12)
        {
            ...
            if (cm != null)
            {
                ...
                {
                    result = new ProxyStatusItem(ProxyStatusProperties.PROXY_ENABLED, CheckStatusValues.CHECKED, false, "status_proxy_mobile_disabled");
                    return result;
                }
            }
        }
        ...
        {
            result = new ProxyStatusItem(ProxyStatusProperties.PROXY_ENABLED, CheckStatusValues.CHECKED, false, "status_proxy_disabled");
        }
        else
        {
            ...
            // {
            // HTTP or SOCKS proxy
            result = new ProxyStatusItem(ProxyStatusProperties.PROXY_ENABLED, CheckStatusValues.CHECKED, true, "status_proxy_enabled");
            // }
            // else
            // {
    ...
    public static ProxyStatusItem isProxyValidHostname(String proxyHost)
    {
        try
        {
            ...
            {
                return new ProxyStatusItem(ProxyStatusProperties.PROXY_VALID_HOSTNAME, CheckStatusValues.CHECKED, false, "status_hostname_empty");
            }
            else
            {
                ...
                if (match.matches())
                {
                    String hostnameValidMsg = "status_hostname_valid";
        ...
        return new ProxyStatusItem(ProxyStatusProperties.PROXY_VALID_HOSTNAME, CheckStatusValues.CHECKED, false, "status_hostname_notvalid");
    }
    ...
    public static ProxyStatusItem isProxyValidExclusionList(String[] proxyExclusionList)
    {
        try
        {
            for (int i = 0; i < proxyExclusionList.length; i++)
            {
                ...
                if (!status.result)
                {
                    return new ProxyStatusItem(ProxyStatusProperties.PROXY_VALID_EXCLUSION_LIST, CheckStatusValues.CHECKED, true, "status_exclusion_list_notvalid");
                }
            }

            String exclusionItemValid = "status_exclusion_item_valid";
        ...
        return new ProxyStatusItem(ProxyStatusProperties.PROXY_VALID_EXCLUSION_ITEM, CheckStatusValues.CHECKED, false, "status_exclusion_item_notvalid");
    }
...
//            {
//                return new ProxyStatusItem(ProxyStatusProperties.PROXY_VALID_EXCLUSION_ITEM, CheckStatusValues.CHECKED, false, "status_exclusion_item_empty"));
//            }
//            else
//            {
            ...
            if (match.matches())
            {
                String msg = String.format("%s %s", "status_exclusion_item_valid", proxyExclusionAddress);
                return new ProxyStatusItem(ProxyStatusProperties.PROXY_VALID_EXCLUSION_ITEM, CheckStatusValues.CHECKED, true, msg);
            }
//            }
        }
    ...
    public static ProxyStatusItem isProxyValidPort(Integer proxyPort)
    {
        if ((proxyPort == null))
        {
            return new ProxyStatusItem(ProxyStatusProperties.PROXY_VALID_PORT, CheckStatusValues.CHECKED, false, "status_port_empty");
        }

        if ((proxyPort < 1) || (proxyPort > 65535))
        {
            return new ProxyStatusItem(ProxyStatusProperties.PROXY_VALID_PORT, CheckStatusValues.CHECKED, false, "status_port_notvalid");
        }

        String msg = String.format("%s %d", "status_port_valid", proxyPort);
    ...
    protected static ProxyStatusItem isProxyReachable(WiFiApConfig conf, int timeout)
    {
        if (conf.getProxy() != null && conf.getProxyType() != Proxy.Type.DIRECT)
        {
            ...
            if (result)
            {
                return new ProxyStatusItem(ProxyStatusProperties.PROXY_REACHABLE, CheckStatusValues.CHECKED, true, "status_proxy_reachable");
            }
            else
            {
                return new ProxyStatusItem(ProxyStatusProperties.PROXY_REACHABLE, CheckStatusValues.CHECKED, false, "status_proxy_not_reachable");
            }
        }
        else
        {
            return new ProxyStatusItem(ProxyStatusProperties.PROXY_REACHABLE, CheckStatusValues.CHECKED, false, "status_proxy_not_valid_informations");
        }
    }
...
//                exList[(i * 2) + 1] = "." + s;
    ...
    protected static ProxyStatusItem isWebReachable(WiFiApConfig conf, int timeout)
    {
        ...
        if (result)
        {
            return new ProxyStatusItem(ProxyStatusProperties.WEB_REACHABLE, CheckStatusValues.CHECKED, true, "status_web_reachable");
        }
        else
        {
            return new ProxyStatusItem(ProxyStatusProperties.WEB_REACHABLE, CheckStatusValues.CHECKED, false, "status_web_not_reachable");
        }
    }
