...
/*
        ...
        final Bitmap scrImgCopy = srcImg;
		...
		if (dest!=null && dstWidth==dest.getWidth() && dstHeight==dest.getHeight()){
			...
			dest.recycle();
			out = Bitmap.createBitmap(dstWidth, dstHeight, Bitmap.Config.ARGB_8888);//new BufferedImage(dstWidth, dstHeight, getResultBufferedImageType(srcImg));
		}
     ...
     * @param workPixels
     */
    private void horizontallyFromSrcToWork(Bitmap srcImg, byte[][] workPixels, int start, int delta) {
		if (nrChannels==1){
     ...
     * @param workPixels
     */
    private void horizontallyFromSrcToWorkGray(Bitmap srcImg, byte[][] workPixels, int start, int delta) {
		final int[] tempPixels = new int[srcWidth];   // Used if we work on int based bitmaps, later used to keep channel values
