    ...
    switch (prefilter->op()) {
      ...
      case Prefilter::AND: {
        ...
        entry->propagate_up_at_count =
            prefilter->op() == Prefilter::AND ? (int)uniq_child.size() : 1;
...
void PrefilterTree::RegexpsGivenStrings(
  ...
  if (!compiled_) {
    ...
    if (!prefilter_vec_.empty()) {
      IntMap regexps_map((int)prefilter_vec_.size());
...
void PrefilterTree::PropagateMatch(const vector<int>& atom_ids,
  ...
  IntMap count((int)entries_.size());
  IntMap work((int)entries_.size());
