...
@interface MPMoPubNativeAdAdapterMF () <MPAdDestinationDisplayAgentDelegateMF>

@property (nonatomic, readonly, retain) MPAdDestinationDisplayAgentMF *destinationDisplayAgent;
@property (nonatomic, assign) UIViewController *rootViewController;
@property (nonatomic, copy) void (^actionCompletionBlock)(BOOL, NSError *);

@end

@implementation MPMoPubNativeAdAdapterMF
...
- (instancetype)initWithAdProperties:(NSMutableDictionary *)properties
{
    if (self = [super init]) {
        ...
        _destinationDisplayAgent = [[[MPCoreInstanceProviderMF sharedProvider] buildMPAdDestinationDisplayAgentWithDelegate:self] retain];
    }
                  ...
                  completion:(void (^)(BOOL success, NSError *error))completionBlock
{
    ...
    if (!controller) {
        NSDictionary *userInfo = [NSDictionary dictionaryWithObject:@"Cannot display content without a root view controller."
                                                             forKey:MPNativeAdErrorContentDisplayErrorReasonKeyMF];
        error = [NSError errorWithDomain:MoPubNativeAdsSDKDomainMF
