...
@interface MPDiskLRUCacheFileMF : NSObject
...
@implementation MPDiskLRUCacheFileMF
...
@implementation MPDiskLRUCacheMF

+ (MPDiskLRUCacheMF *)sharedDiskCache
{
    static dispatch_once_t once;
    static MPDiskLRUCacheMF *sharedDiskCache;
    dispatch_once(&once, ^{
...
- (void)removeAllCachedFiles
{
    dispatch_sync(self.diskIOQueue, ^{
        ...
        for (MPDiskLRUCacheFileMF *file in allFiles) {
...
- (void)ensureCacheSizeLimit
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        MPLogDebugMF(@"Checking cache size...");
        ...
        dispatch_async(self.diskIOQueue, ^{
            @autoreleasepool {
                ...
                for (MPDiskLRUCacheFileMF *file in expiredFiles) {
                    MPLogDebugMF(@"Trying to remove %@ from cache due to expiration", file.filePath);
                ...
                while ([self sizeOfCacheFilesInArray:cacheFilesSortedByModDate] >= kCacheSoftMaxSize && cacheFilesSortedByModDate.count > 0) {
                    NSString *oldestFilePath = ((MPDiskLRUCacheFileMF *)[cacheFilesSortedByModDate objectAtIndex:0]).filePath;
                    
                    MPLogDebugMF(@"Trying to remove %@ from cache due to size", oldestFilePath);
...
- (NSArray *)expiredCachedFilesInArray:(NSArray *)cachedFiles
{
    ...
    for (MPDiskLRUCacheFileMF *file in cachedFiles) {
...
- (NSMutableArray *)cacheFilesSortedByModDate
{
    ...
    for (NSString *fileName in sortedFiles) {
        ...
        MPDiskLRUCacheFileMF *cacheFile = [[[MPDiskLRUCacheFileMF alloc] init] autorelease];
...
- (uint64_t)sizeOfCacheFilesInArray:(NSArray *)files
{
    ...
    for (MPDiskLRUCacheFileMF *file in files) {
        currentSize += file.fileSize;
    }
    
    MPLogDebugMF(@"Current cache size %qu bytes", currentSize);
...
{
    NSString *hashedKey = MPSHA1DigestMF(key);
