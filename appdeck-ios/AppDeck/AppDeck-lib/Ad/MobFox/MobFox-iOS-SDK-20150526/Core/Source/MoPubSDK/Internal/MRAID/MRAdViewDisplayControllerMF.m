...
#import "MRAdViewMF+Controllers.h"
...
@interface MRAdViewDisplayControllerMF ()

@property (nonatomic, retain) MRAdViewMF *twoPartExpansionView;
...
@implementation MRAdViewDisplayControllerMF
     ...
     jsEventEmitter:(MRJavaScriptEventEmitterMF *)jsEventEmitter
{
    ...
    if (self) {
        ...
        _viewabilityTimer = [[[MPCoreInstanceProviderMF sharedProvider] buildMPTimerWithTimeInterval:kViewabilityTimerInterval
                                                                                        target:self
        ...
        _dimmingView = [[MRDimmingViewMF alloc] initWithFrame:MPKeyWindowMF().frame];
...
- (void)initializeJavascriptStateWithViewProperties:(NSArray *)viewProperties
{
    ...
    properties = [properties arrayByAddingObjectsFromArray:[NSArray arrayWithObjects:
            [MRScreenSizePropertyMF propertyWithSize:MPApplicationFrameMF().size],
            [MRStatePropertyMF propertyWithState:_currentState],
            nil]];
...
- (CGRect)defaultPosition {
    UIWindow *keyWindow = MPKeyWindowMF();
...
- (void)expandToFrame:(CGRect)frame withURL:(NSURL *)url blockingColor:(UIColor *)blockingColor
    ...
    [MPKeyWindowMF() addSubview:_dimmingView];
    
    if (url) {
        self.twoPartExpansionView = [[MPInstanceProviderMF sharedProvider] buildMRAdViewWithFrame:self.view.frame
                                                                                allowsExpansion:NO
...
- (void)applyRotationTransformForCurrentOrientationOnView:(UIView *)view {
    ...
    switch (MPInterfaceOrientationMF()) {
...
- (void)assignRandomTagToDefaultSuperview {
    ...
    } while ([MPKeyWindowMF() viewWithTag:_parentTag]);
    
    self.view.superview.tag = _parentTag;
}

- (void)restoreDefaultSuperviewTag {
    [[MPKeyWindowMF() viewWithTag:_parentTag] setTag:_originalTag];
}
...
- (void)moveViewFromWindowToDefaultSuperview {
    UIView *defaultSuperview = [MPKeyWindowMF() viewWithTag:_parentTag];
...
- (void)constrainViewBoundsToApplicationFrame {
    ...
    CGRect applicationFrame = MPApplicationFrameMF();
...
- (CGRect)orientationAdjustedRect:(CGRect)rect {
    ...
    UIInterfaceOrientation orientation = MPInterfaceOrientationMF();
...
- (void)expandAnimationDidStop {
    ...
    [self.jsEventEmitter fireChangeEventForProperty:[MRStatePropertyMF propertyWithState:_currentState]];
...
- (void)updateViewabilityWithBool:(BOOL)currentViewability {
    if (_isViewable != currentViewability) {
        MPLogDebugMF(@"Viewable changed to: %@", currentViewability ? @"YES" : @"NO");
