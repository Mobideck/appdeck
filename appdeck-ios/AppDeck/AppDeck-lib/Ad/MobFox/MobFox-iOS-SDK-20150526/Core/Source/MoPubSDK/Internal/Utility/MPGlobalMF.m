...
BOOL MPViewHasHiddenAncestorMF(UIView *view);
...
UIInterfaceOrientation MPInterfaceOrientationMF()
{
    return [UIApplication sharedApplication].statusBarOrientation;
}

UIWindow *MPKeyWindowMF()
{
    return [UIApplication sharedApplication].keyWindow;
}

CGFloat MPStatusBarHeightMF() {
...
CGRect MPApplicationFrameMF()
{
    CGRect frame = MPScreenBoundsMF();

    frame.origin.y += MPStatusBarHeightMF();
    frame.size.height -= MPStatusBarHeightMF();

    return frame;
}

CGRect MPScreenBoundsMF()
{
    CGRect bounds = [UIScreen mainScreen].bounds;

    if (UIInterfaceOrientationIsLandscape(MPInterfaceOrientationMF()))
    {
...
BOOL MPViewIsVisibleMF(UIView *view)
{
    ...
    return (!view.hidden &&
            !MPViewHasHiddenAncestorMF(view) &&
...
@implementation NSString (MPAdditionsMF)
...
@implementation UIDevice (MPAdditionsMF)
...
@interface MPTelephoneConfirmationControllerMF ()
...
@implementation MPTelephoneConfirmationControllerMF
...
- (id)initWithURL:(NSURL *)url clickHandler:(MPTelephoneConfirmationControllerClickHandler)clickHandler
{
    if (![url mp_hasTelephoneScheme] && ![url mp_hasTelephonePromptScheme]) {
        ...
        MPLogErrorMF(@"Processing URL as a telephone URL when %@ doesn't follow the tel:// or telprompt:// schemes", url.absoluteString);
        [self release];
        return nil;
    }
    ...
    if (self = [super init]) {
        ...
        if (!phoneNumber) {
            ...
            if ([phoneNumber length] == 0) {
                MPLogErrorMF(@"Invalid telelphone URL: %@.", url.absoluteString);
                [self release];
                return nil;
            }
        }
