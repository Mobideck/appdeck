...
@implementation MRCalendarManagerMF
...
- (id)initWithDelegate:(id<MRCalendarManagerDelegateMF>)delegate
{
    if (self = [super init]) {
        _delegate = delegate;
...
- (void)dealloc
{
    ...
    // XXX:
    [_eventEditViewController setEditViewDelegate:[MPLastResortDelegateMF sharedDelegate]];
...
- (void)createCalendarEventWithParameters:(NSDictionary *)parameters
{
    if (!self.eventEditViewController) {
        self.eventEditViewController = [[MPInstanceProviderMF sharedProvider] buildEKEventEditViewControllerWithEditViewDelegate:self];
    }
