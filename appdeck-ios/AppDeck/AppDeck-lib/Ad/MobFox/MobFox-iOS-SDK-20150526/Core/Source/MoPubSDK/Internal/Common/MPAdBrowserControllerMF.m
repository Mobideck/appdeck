...
@interface MPAdBrowserControllerMF ()
...
@implementation MPAdBrowserControllerMF
...
{
    if (self = [super initWithNibName:@"MPAdBrowserControllerMF" bundle:nil])
    {
        self.delegate = delegate;
        self.URL = URL;
        self.HTMLString = HTMLString;

        MPLogDebugMF(@"Ad browser (%p) initialized with URL: %@", self, self.URL);
...
{
    MPLogDebugMF(@"Ad browser (%p) starting to load URL: %@", self, request.URL);
...
- (void)webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error
{
    ...
    MPLogErrorMF(@"Ad browser (%p) experienced an error: %@.", self, [error localizedDescription]);
}
