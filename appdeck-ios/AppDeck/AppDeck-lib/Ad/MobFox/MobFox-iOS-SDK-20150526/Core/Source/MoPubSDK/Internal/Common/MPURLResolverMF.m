...
@implementation MPURLResolverMF
...
{
    return [[[MPURLResolverMF alloc] init] autorelease];
}
...
- (void)startResolvingWithURL:(NSURL *)URL delegate:(id<MPURLResolverDelegateMF>)delegate
{
    ...
    if (![self handleURL:self.URL]) {
        NSURLRequest *request = [[MPCoreInstanceProviderMF sharedProvider] buildConfiguredURLRequestWithURL:self.URL];
...
- (NSStringEncoding)stringEncodingFromContentType:(NSString *)contentType
{
    ...
    if (![contentType length]) {
        MPLogWarnMF(@"Attempting to set string encoding from nil %@", kMoPubHTTPHeaderContentType);
        return encoding;
    }
