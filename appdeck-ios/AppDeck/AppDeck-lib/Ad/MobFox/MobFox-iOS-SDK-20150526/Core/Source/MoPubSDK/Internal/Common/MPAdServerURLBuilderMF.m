...
@interface MPAdServerURLBuilderMF ()
...
@implementation MPAdServerURLBuilderMF
...
+ (NSString *)queryParameterForKeywords:(NSString *)keywords
{
    ...
    if ([fbKeywordProviderClass conformsToProtocol:@protocol(MPKeywordProviderMF)])
    {
        NSString *fbAttributionKeyword = [(Class<MPKeywordProviderMF>) fbKeywordProviderClass keyword];
...
+ (NSString *)queryParameterForScaleFactor
{
    return [NSString stringWithFormat:@"&sc=%.1f", MPDeviceScaleFactorMF()];
}
...
+ (NSString *)queryParameterForConnectionType
{
    return [[[MPCoreInstanceProviderMF sharedProvider] sharedMPReachability] hasWifi] ? @"&ct=2" : @"&ct=3";
}
...
+ (NSString *)queryParameterForCarrierName
{
    NSString *carrierName = [[[MPCoreInstanceProviderMF sharedProvider] sharedCarrierInfo] objectForKey:@"carrierName"];
...
+ (NSString *)queryParameterForISOCountryCode
{
    NSString *code = [[[MPCoreInstanceProviderMF sharedProvider] sharedCarrierInfo] objectForKey:@"isoCountryCode"];
    return code ? [NSString stringWithFormat:@"&iso=%@", [code URLEncodedString]] : @"";
}

+ (NSString *)queryParameterForMobileNetworkCode
{
    NSString *code = [[[MPCoreInstanceProviderMF sharedProvider] sharedCarrierInfo] objectForKey:@"mobileNetworkCode"];
    return code ? [NSString stringWithFormat:@"&mnc=%@", [code URLEncodedString]] : @"";
}

+ (NSString *)queryParameterForMobileCountryCode
{
    NSString *code = [[[MPCoreInstanceProviderMF sharedProvider] sharedCarrierInfo] objectForKey:@"mobileCountryCode"];
...
+ (NSString *)queryParameterForTwitterAvailability
{
    MPTwitterAvailability twitterAvailability = [[MPCoreInstanceProviderMF sharedProvider] twitterAvailabilityOnDevice];
...
+ (BOOL)advertisingTrackingEnabled
{
    return [MPIdentityProviderMF advertisingTrackingEnabled];
}
