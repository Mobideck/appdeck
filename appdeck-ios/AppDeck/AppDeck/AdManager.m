...
//#import "Ad/API/APIAdEngine.h"
//#import "Ad/MMedia/MMediaAdEngine.h"
#import "Ad/MoPub/MoPubAdEngine.h"
#import "Ad/AppsFire/AppsFireAdEngine.h"
#import "Ad/HTMLChunk/HTMLChunkAdEngine.h"
...
#import "Ad/Swelen/SwelenAds-50000c/SwelenSDK/swAdAPI.h"
#import "Ad/AdMob/GoogleMobileAdsSdkiOS-6.9.2/GADBannerView.h"
#import "Ad/InMobi/InMobi-iOS-SDK-4.3.0/Libs/InMobi.h"
...
-(id)initWithLoader:(LoaderViewController *)loader
{
    ...
    // swelen init
    //SW_LOAD_ADMOB();
    //SW_LOAD_INMOBI(@"4f225cb3cbd84c85a6649702ffa2adab");
...
//        self.fakeCtl = [[UIViewController alloc] init];
    ...
    /*
    NSString *adConfUrl = [NSString stringWithFormat:@"http://adv2.appdeck.mobi/conf/%@", self.loader.conf.app_api_key];

    jsonapi = [JSonHTTPApi apiWithURL:adConfUrl params:nil callback:^(NSDictionary *result, NSError *error)
     {
         ...
         if (error != nil)
         {
             ...
             NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:adConfUrl] cachePolicy:NSURLRequestReturnCacheDataDontLoad timeoutInterval:60.0];
             jsonapi = [JSonHTTPApi apiWithRequest:request callback:^(NSDictionary *result, NSError *error) {
                 if (error != nil)
                 {
                     NSLog(@"Cached AdConf Error: %@ - %@", result, error);
                     return;
                 }
                 [self loadAdConf:result];
             }];
             return;
         }
         [self loadAdConf:result];
...
/*    [jsonapi cancel];
    ...
    [self.fakeCtl.view removeFromSuperview];
    [self.fakeCtl removeFromParentViewController];

    for (AppDeckAdViewController *ad in bannerAds)
    {
        [ad cancel];
        PageViewController *page = (PageViewController *)ad.page;
        if (page && page.bannerAd == ad)
            page.bannerAd = nil;
        ad.page = nil;
    }

    for (AppDeckAdViewController *ad in rectangleAds)
    {
        PageViewController *page = (PageViewController *)ad.page;
        if (page && page.rectangleAd == ad)
            page.rectangleAd = nil;
        ad.page = nil;
    }
    
    for (AppDeckAdViewController *ad in interstitialAds)
    {
        PageViewController *page = (PageViewController *)ad.page;
        if (page && page.interstitialAd == ad)
...
/*
-(void)loadAdConf:(NSDictionary *)conf
{
    NSDictionary *adConfigs = [conf objectForKey:@"config"];
    NSDictionary *adNetworks = [conf objectForKey:@"adnetwork"];
    
    // no ad config ?
    if (adConfigs == nil || [adConfigs respondsToSelector:@selector(objectForKey:)] == NO)
        return;
    if (adNetworks == nil || [adNetworks respondsToSelector:@selector(objectAtIndex:)] == NO)
        return;
    
    if ([conf objectForKey:@"enable_debug"] && [[conf objectForKey:@"banner"] boolValue])
        self.enable_debug = YES;
    
    self.adTypes = [[NSMutableArray alloc] init];
    self.adConfigs = [[NSMutableDictionary alloc] init];
    self.adUsages = [[NSMutableDictionary alloc] init];
    self.adPreloads = [[NSMutableDictionary alloc] init];

    // init all ad type defined in configuration
//    NSMutableDictionary *sortedAdTypes = [[NSMutableDictionary alloc] init];
    for (NSString* adType in adConfigs)
    {
        AppDeckAdConfig *adConfig = [AppDeckAdConfig adConfigFronJson:[adConfigs objectForKey:adType]];
        ...
        [self.adPreloads setObject:[[AppDeckAdPreload alloc] init] forKey:adType];
    }
    
    // sort adType
    NSArray* values = [self.adConfigs allValues];
    NSArray *sortedValues = [values sortedArrayUsingFunction:adConfigSortAsc context:nil];
    for (AppDeckAdConfig *adConfig in sortedValues)
    {
        [self.adTypes addObject:adConfig.adType];
    }
    
    // init all ad networks defined in configuration
    for (NSDictionary *adNetworkConfig in adNetworks)
    {
        if (adNetworkConfig == nil || [adNetworkConfig respondsToSelector:@selector(objectForKey:)] == NO)
            continue;

        NSString *engineName = [NSString stringWithFormat:@"%@", [adNetworkConfig objectForKey:@"engine"]];
        
        AppDeckAdEngine *engine = nil;
        if ([engineName isEqualToString:@"fake"] && NSClassFromString(@"FakeAdEngine") != nil)
            engine = [[NSClassFromString(@"FakeAdEngine") alloc] initWithAdManager:self andConfiguration:adNetworkConfig];
        else if ([engineName isEqualToString:@"api"] && NSClassFromString(@"APIAdEngine") != nil)
            engine = [[NSClassFromString(@"APIAdEngine") alloc] initWithAdManager:self andConfiguration:adNetworkConfig];
        else if ([engineName isEqualToString:@"mopub"] && NSClassFromString(@"MoPubAdEngine") != nil)
            engine = [[NSClassFromString(@"MoPubAdEngine") alloc] initWithAdManager:self andConfiguration:adNetworkConfig];
        else if ([engineName isEqualToString:@"mmedia"] && NSClassFromString(@"MMediaAdEngine") != nil)
            engine = [[NSClassFromString(@"MMediaAdEngine") alloc] initWithAdManager:self andConfiguration:adNetworkConfig];
        else if ([engineName isEqualToString:@"widespace"] && NSClassFromString(@"WideSpaceAdEngine") != nil)
            engine = [[NSClassFromString(@"WideSpaceAdEngine") alloc] initWithAdManager:self andConfiguration:adNetworkConfig];
        else if ([engineName isEqualToString:@"appsfire"] && NSClassFromString(@"AppsFireAdEngine") != nil)
            engine = [[NSClassFromString(@"AppsFireAdEngine") alloc] initWithAdManager:self andConfiguration:adNetworkConfig];
        
        if (engine != nil)
            [adEngines addObject:engine];
    }

    // conf loaded, launch start event
    LoaderChildViewController *child = [self.loader getCurrentChild];
    [self pageViewController:(PageViewController *)child appearWithEvent:AdManagerEventLaunch];
...
-(void)checkAds:(NSTimer *)timer
{
    ...
    if (![child isKindOfClass:[PageViewController class]])
        return;
    
    PageViewController *page = (PageViewController *)child;
    
    [self pageViewController:page appearWithEvent:AdManagerEventNone];
}

#pragma mark - Load ads

-(void)tryNextAdWithType:(NSString *)adType
{
    AppDeckAdPreload *adPreload = [self.adPreloads objectForKey:adType];
    if (adPreload == nil)
        return;
    
    if (adPreload.AdEngineChain.count == 0)
    {
        adPreload.AdEngineChain = nil;
        return;
    }
    
    AppDeckAdEngine *engine = [adPreload.AdEngineChain objectAtIndex:0];
    [adPreload.AdEngineChain removeObjectAtIndex:0];
    
    // check usage
    AppDeckAdUsage *adUsage = [engine.adUsages objectForKey:adType];
    if ([adUsage shouldFetchAdForPageViewController:nil appearingWithEvent:adPreload.originEvent] == NO)
        return [self tryNextAdWithType:adType];
    
    // fetch ad
    AppDeckAdViewController *ad = [engine createAdWithType:adType];
    if (ad == nil)
        return;
    
    // preload this ad
    adPreload.workingAd = ad;
    [self.fakeCtl addChildViewController:ad];
    [self.fakeCtl.view addSubview:ad.view];

}

NSInteger adEngineSortAsc(AppDeckAdEngine *engine1, AppDeckAdEngine *engine2, void *context)
{
    NSString *adType = (__bridge NSString *)context;

    AppDeckAdConfig *adConfig1 = [engine1.adConfigs objectForKey:adType];
    AppDeckAdConfig *adConfig2 = [engine2.adConfigs objectForKey:adType];

    if (adConfig1 == nil && adConfig2 == nil)
    ...
    return [adConfig1 compare:adConfig2];
}

NSInteger adConfigSortAsc(AppDeckAdConfig *adConfig1, AppDeckAdConfig *adConfig2, void *context)
{
    if (adConfig1 == nil && adConfig2 == nil)
        return NSOrderedSame;
    if (adConfig1 == nil)
...
-(void)preloadAdType:(NSString *)adType WithEvent:(AdManagerEvent)event
{
    AppDeckAdPreload *adPreload = [self.adPreloads objectForKey:adType];
    if (adPreload == nil)
        return;
    
    // an ad is already being loaded
    if (adPreload.AdEngineChain)
        return;

    adPreload.originEvent = event;
    
    NSArray *sortedAdEngines = [adEngines sortedArrayUsingFunction:adEngineSortAsc context:(__bridge void *)(adType)];
    
    adPreload.AdEngineChain = [NSMutableArray arrayWithArray:sortedAdEngines];
...
//    NSString *adConfUrl = @"http://test.cloud.oxom.com/ads/api/template/get";
    
    NSMutableDictionary *adContext = [[NSMutableDictionary alloc] init];
    [adContext setObject:page.url.absoluteString forKey:@"url"];
    ...
    // request
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:adConfUrl] cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:60.0];
    ...
    page.adRequest = [JSonHTTPApi apiWithRequest:request callback:^(NSDictionary *result, NSError *error) {
        if (error != nil)
        {
            NSLog(@"AdConf Error: %@ - %@", result, error);
            page.adRequest = nil;
    ...
    for (NSString *adType in self.adTypes)
    {
        ...
        if ([adUsage shouldFetchAdForPageViewController:page appearingWithEvent:event] == YES)
        {
            AppDeckAdPreload *adPreload = [self.adPreloads objectForKey:adType];
            if (adPreload == nil)
                continue;
            
            if (adPreload.readyAd)
            {
                // remove from fakeCtl
                [adPreload.readyAd removeFromParentViewController];
                [adPreload.readyAd.view removeFromSuperview];
                
                // set in page
                if ([adType isEqualToString:@"banner"])
                    page.bannerAd = adPreload.readyAd;
                else if ([adType isEqualToString:@"rectangle"])
                    page.rectangleAd = adPreload.readyAd;
                else if ([adType isEqualToString:@"interstitial"])
                    page.interstitialAd = adPreload.readyAd;

                adPreload.readyAd.state = AppDeckAdStateLoad;
                adPreload.readyAd = nil;
                
                return;
            } else
                [self preloadAdType:(NSString *)adType WithEvent:event];
        }
    }*/
...
-(AppDeckAdEngine *)adEngineFromId:(NSString *)engineId type:(NSString *)type config:(NSDictionary *)config
{
    ...
    if ([type isEqualToString:@"html_chunk"])
    {
        adEngine = [[HTMLChunkAdEngine alloc] initWithAdManager:self andConfiguration:config];
    }
    else if ([type isEqualToString:@"widespace"])
    {
        adEngine = [[WideSpaceAdEngine alloc] initWithAdManager:self andConfiguration:config];
    }
    else if ([type isEqualToString:@"appsfire"])
    {
        adEngine = [[AppsFireAdEngine alloc] initWithAdManager:self andConfiguration:config];
    }
    else if ([type isEqualToString:@"mopub"])
    {
        adEngine = [[MoPubAdEngine alloc] initWithAdManager:self andConfiguration:config];
    }
