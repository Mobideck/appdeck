...
describe(@"MPAdAlertGestureRecognizer", ^{
    ...
    beforeEach(^{
        ...
        model = [[[MPAdAlertGestureRecognizer alloc] initWithTarget:gestureResponder action:@selector(handleGestureRecognized)] autorelease];
        
        view = [[[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, 250)] autorelease];
    ...
    context(@"when the user zig zags enough times to trigger the gesture", ^{
       it(@"should notify the target that the gesture has been recognized", ^{
           ...
           gestureResponder should have_received(@selector(handleGestureRecognized));
       });
    });
